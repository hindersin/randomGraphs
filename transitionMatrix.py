...
from scipy.sparse import coo_matrix # Efficient for incremental construction of sparse matrix, convert to csr_matrix for operations on matrix.

def createTransitionMatrix(fitnessMutants, popSize, graph, update='BD'):
    '''
        ...
        for j in xrange(0, numStates):
            ...
            if update == 'BD':
                ...
                if (sum(abs(state_i - state_j)) == 1):
                    # Increasing the number of mutants. If sum is -1, then state j has one more mutant than i.
                    if (sum(state_i - state_j) == -1):
                        ...
                        for k in xrange(0, numMutants):
                            # has_edge returns True if those nodes are connected. 
                    ...
                    # Decreasing the number of mutants. If sum is 1, then state j has one less mutant than state i.
                        ...
                        for k in xrange(0, len(wilds)):
                            prob = prob + float(graph.has_edge(changingPosition, wilds[k])) / float(len(graph.neighbors(wilds[k])))
                ...
                # Check that state j can be reached within one time step
                if (sum(abs(state_i - state_j)) == 1):
                    # If sum is -1, then state j has one more mutant than i.
                    if (sum(state_i - state_j) == -1):
                        mutants = np.where(state_i == 1)[0]
                        changingPosition = np.where((state_i - state_j) == -1)[0][0]
                        neighborsOfChanging = graph.neighbors(changingPosition)
